...
class ColumnAdverts extends Module
{
    ...
    public function __construct()
    {
        ...
        parent::__construct();
        $this->displayName = $this->l('Left column adverts');
        $this->description = $this->l('Display image adverts on left column');
    }

    public function install()
    {
        $this->_clearCache('columnadverts.tpl');
        ...
        Configuration::updateValue($this->name . '_slider', 0);
    ...
    private function _postProcess()
    {
        ...
        Configuration::updateValue($this->name . '_slider', Tools::getValue('slider'));
    ...
    public function getContent()
    {
        ...
        $html .= '
			...
			<script type="text/javascript" src="../modules/' . $this->name . '/columnadverts.js"></script>
	...
	<label class="control-label col-lg-3 ">' . $this->l('Show images as slider') . '</label>
		<div class="col-lg-9">
	<span class="switch prestashop-switch fixed-width-lg">
		<input type="radio" name="slider" id="slider_on" value="1" ' . ((Configuration::get($this->name . '_slider') == 1) ? 'checked="checked" ' : '') . '>
		<label for="slider_on">
			Yes
		</label>
		<input type="radio" name="slider" id="slider_off" value="0" ' . ((Configuration::get($this->name . '_slider') == 0) ? 'checked="checked" ' : '') . '>
		<label for="slider_off">
			No
		</label>
    ...
    public function hookLeftColumn($params)
    {
        ...
        if (!$this->isCached('columnadverts.tpl', $this->getCacheId())) {

            $slider = Configuration::get($this->name . '_slider');

            $this->context->smarty->assign(array(
                'climages' => $this->_getImageArray(true, true),
                'slider' => $slider,
            ));
        }
        return $this->display(__FILE__, 'columnadverts.tpl', $this->getCacheId());
    ...
    {
        $this->context->controller->addCSS(($this->_path) . 'columnadverts.css', 'all');
        $this->context->controller->addJS(($this->_path) . 'columnadvertsfront.js');
    }

    public function ajaxProcessDeleteImage($image)
    {
        $image = $_GET['image'];
    ...
    public function ajaxProcessUploadImage($userfile)
    {
